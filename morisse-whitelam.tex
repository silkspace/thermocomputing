...
\begin{figure}[h]
\centering
\includegraphics[width=\textwidth]{paper_figures/trajectory_fast.png}
\caption{Relaxation trajectory under the $\varphi^4$+bias potential. Starting from random noise (t=0), the system rapidly converges to the digit-3 attractor. Structure is visible by t=5, clear by t=10, and stable by t=20. This fast convergence ($\sim$10--20 steps vs.\ hundreds in naive implementations) reflects the strong restoring forces of the $\varphi^4$ bistable wells combined with learned biases.}
...
\textbf{Attractor visualization:} Starting from uniform gray ($x_i = 0$) and evolving under $V_c$, pixels organize into the pattern for digit $k$.
...
\begin{equation}
k^* = \arg\min_k V_c(\mathbf{x}) \quad \text{or equivalently} \quad k^* = \arg\min_k \|\nabla V_c(\mathbf{x})\|^2.
\end{equation}
...
\item \textbf{Recognize}: Evaluate $V_c(\mathbf{x})$ for all $k$ (parallelizable across bias banks). Select $k^* = \arg\min_k V_c(\mathbf{x})$.
...
The trajectory action (Section~\ref{sec:constraint}) learns each $\mathbf{b}_c$ by projecting onto the constraint surface $\mathcal{C}_c$ where class-$c$ trajectories are maximally likely.
...
\begin{equation}
L_{\rm contrastive} = \sum_k \left[ V_c(\mathbf{x}_c) - \frac{1}{K-1}\sum_{j \neq k} V_j(\mathbf{x}_c) \right],
...
\emph{For shared couplings $J_{ij}$}: Since $\partial V_c / \partial J_{ij} = x_i x_j$ is independent of $k$, we have
\begin{equation}
\frac{\partial L_{\rm contrastive}}{\partial J_{ij}} = \sum_k \left[ x_{c,i} x_{k,j} - \frac{K-1}{K-1} x_{c,i} x_{k,j} \right] = 0.
\end{equation}
...
\begin{equation}
d\mathbf{x} = -\nabla V_c \, dt + \sqrt{2k_{\rm B}T}\, d\mathbf{W}, \quad p(\mathbf{x}|k) \propto e^{-V_c(\mathbf{x})/k_{\rm B}T},
\end{equation}
the contrastive objective provides detailed balance in \emph{class space}. Define
\begin{equation}
p(k|\mathbf{x}) = \frac{\exp(-V_c(\mathbf{x})/T)}{\sum_j \exp(-V_j(\mathbf{x})/T)},
\label{eq:class_softmax}
\end{equation}
a Boltzmann distribution over discrete class labels. At low temperature $T \to 0$, this concentrates on $k^* = \arg\min_k V_c(\mathbf{x})$. The contrastive loss ensures this ``classification dynamics'' has the correct fixed point: data from class $c$ should flow to class $c$.
...
\begin{equation}
p(\mathbf{x}, k) \propto \exp\left(-V_c(\mathbf{x})/k_{\rm B}T\right).
\label{eq:joint}
\end{equation}
...
\begin{equation}
p(\mathbf{x}) = \sum_k p(\mathbf{x},k) \propto \sum_k \exp(-V_c(\mathbf{x})/k_{\rm B}T),
\end{equation}
...
The contrastive signal $-b_c + \frac{1}{K-1}\sum_{j\neq k} b_j$ reveals what makes each digit \emph{unique}---the discriminative features that separate it from other classes.
