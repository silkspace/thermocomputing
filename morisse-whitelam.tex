...
The core idea is simple: we want parameters such that running the system \emph{backward} from noise reconstructs data.
Detailed balance provides a shortcut---maximizing the likelihood of observed \emph{forward} trajectories automatically maximizes backward (reconstructive) likelihood.
This yields closed-form gradient estimators for both biases $b_i$ and couplings $J_{ij}$, where the gradients depend on the mismatch between observed and model-predicted velocities.
Unlike Equilibrium Propagation, which requires two equilibration phases (free and nudged), our approach needs only \emph{single-pass observation} of trajectories---no equilibration, no nudging, no negative-phase sampling.
The Onsager-Machlup action provides theoretical grounding, revealing that learning is \emph{constraint satisfaction}: correct parameters minimize trajectory action to its thermal floor.
We demonstrate that coupling learning captures \emph{genuine second-order structure}: on within-class centered MNIST data (where nearest-centroid classification achieves only 9.4\%, equivalent to random guessing), learned couplings achieve 78.8\% accuracy---proving the method extracts correlational structure beyond simple templates.
We also show $>$99\% parameter recovery in 10--20 epochs---fast convergence because each trajectory directly constrains parameters rather than requiring iterative search.
CMOS p-bits biased at threshold naturally implement these dynamics, and the local gradient formulas enable fully on-chip learning.
We present a hardware blueprint establishing a route toward trainable thermodynamic neural computers.
\end{abstract}
...
The core idea is direct: we seek parameters such that running the system \emph{backward} from noise reconstructs data.
Detailed balance provides a crucial shortcut---maximizing the likelihood of observed forward trajectories automatically maximizes backward (reconstructive) likelihood---yielding closed-form gradient estimators from single-pass trajectory observation.
This differs fundamentally from Equilibrium Propagation~\cite{scellier2017ep}, which requires two equilibration phases, and from Boltzmann machines, which require negative-phase sampling.
We need neither: just observe the trajectory, compute the velocity residual, and update.

The Onsager-Machlup action~\cite{onsager1953fluctuations} provides theoretical grounding for why this works.
Learning becomes \emph{constraint satisfaction}: correct parameters minimize trajectory action to its thermal floor, making observed dynamics maximally likely.
This explains the rapid convergence---10--20 epochs rather than thousands---because projecting onto a constraint surface is geometrically simpler than searching a high-dimensional loss landscape.
...
(iv) Empirical validation showing that coupling learning captures genuine second-order structure: 78.8\% accuracy on centered MNIST data where nearest-centroid classification achieves only 9.4\%.
...
\textbf{Training and generation in thermodynamic systems.}
Whitelam~\cite{whitelam2025training} showed that thermodynamic computers can be trained via gradient descent using a teacher-student framework, reporting potential energy advantages of seven orders of magnitude.
The same author introduced generative thermodynamic computing~\cite{whitelam2025generative}, where structured data emerges from noise through Langevin dynamics without external neural networks.
Our work complements these approaches by providing closed-form gradient estimators derived from the Onsager-Machlup action, enabling fully local on-chip learning without teacher networks.
...
The current MNIST demonstration uses 14$\times$14 downsampled images; scaling to larger state spaces requires investigation.
Generation quality with couplings alone (without biases) produces structured but noisy outputs, suggesting that first-order and second-order structure play complementary roles.
...
However, a key insight improves this dramatically: \textbf{coupling learning with thermal pixel exchanges achieves 78.8\% test accuracy on within-class centered data}---where nearest-centroid classification achieves only 9.4\%.
...
\textbf{Denoising as trajectory observation.}
A key insight connects score matching to trajectory-based learning: \emph{denoising IS trajectory observation}.
When we add noise to an MNIST image $\mathbf{x}$ to obtain a corrupted version $\mathbf{y} = \mathbf{x} + \boldsymbol{\epsilon}$, the denoising direction $\mathbf{x} - \mathbf{y} = -\boldsymbol{\epsilon}$ is exactly the trajectory step we would observe if the system relaxed from $\mathbf{y}$ back toward equilibrium at $\mathbf{x}$.

For a quadratic energy $V(\mathbf{x}) = \frac{1}{2}\mathbf{x}^\top J \mathbf{x}$, the predicted drift is $-\nabla V = -J\mathbf{x}$.
At the noisy point $\mathbf{y}$, the model score is $-J\mathbf{y}$.
Score matching trains $J$ so that this predicted direction matches the true denoising direction $-\boldsymbol{\epsilon}/\sigma^2$.
This is mathematically equivalent to observing a single Langevin step from $\mathbf{y}$ toward $\mathbf{x}$ and applying the gradient formula (\ref{eq:J_grad}).

Figure~\ref{fig:mnist_thermalization} shows this process: MNIST digits thermalize under Langevin dynamics, with pixel intensities fluctuating around their equilibrium values.
The trajectories of this thermalization---the sequence of displacements $\Delta\mathbf{x}^k$---encode information about the energy landscape that the gradient formulas extract.

\begin{figure}[h]
\centering
\includegraphics[width=\textwidth]{paper_figures/mnist_thermalization.png}
\caption{MNIST digits thermalizing under Langevin dynamics. Each row shows a different digit (0--9) evolving from equilibrium ($t=0$) through increasing thermal fluctuation. The trajectory of displacements encodes information about the energy landscape: pixels that co-vary under thermal noise are coupled in the learned $J$ matrix. This visualization demonstrates the physical interpretation of trajectory-based learning: we observe how the system fluctuates and extract parameters from the statistics of these fluctuations.}
\label{fig:mnist_thermalization}
\end{figure}

\textbf{Thermal pixel exchanges (Kawasaki dynamics).}
An alternative formulation uses mass-conserving dynamics where pixels \emph{exchange} intensity rather than receiving independent noise.
At each timestep, we randomly select pixel pairs $(i,j)$ and transfer a small amount of intensity from one to the other:
\begin{equation}
x_i \to x_i - \delta, \quad x_j \to x_j + \delta, \quad \text{where } \delta \sim \mathcal{N}(0, \sigma^2).
\end{equation}
This conserves total mass $\sum_i x_i$ exactly, mimicking Kawasaki spin-exchange dynamics in statistical physics.
The key insight is that correlated pixels---those that tend to have similar values within a class---will show characteristic exchange patterns.
If pixels $i$ and $j$ are both typically bright (or both typically dark) in digit ``3'', exchanging intensity between them produces smaller net changes than exchanging between a bright and dark pixel.
These exchange statistics encode \emph{second-order structure}---the correlations $\langle x_i x_j \rangle$ rather than just the means $\langle x_i \rangle$.

\textbf{Bias-only learning: A baseline (and its limitations).}
If we learn only the biases $\mathbf{b}_c$ while keeping couplings $J_{ij} = 0$, the gradient estimator (\ref{eq:b_grad}) reduces to:
\begin{equation}
\Delta b_{c,i} \propto -\frac{1}{N_c} \sum_{\text{samples of class } c} x_i = -\langle x_i \rangle_c.
\end{equation}
This means the learned bias is simply the \emph{negative class mean}---the average digit template.
Classification reduces to nearest-centroid matching: $c^* = \arg\min_c \|\mathbf{x} - \boldsymbol{\mu}_c\|^2$, which achieves $\sim$82\% accuracy on raw MNIST.
While this validates the gradient formula, it is not learning anything beyond first-order statistics (means).

\textbf{The critical test: within-class centering.}
To verify that coupling learning captures \emph{genuine second-order structure}, we apply within-class centering: for each class $c$, we subtract the class mean $\boldsymbol{\mu}_c$ from all training and test samples of that class.
After centering, all class centroids are identically zero, and nearest-centroid classification achieves only $\sim$9.4\%---equivalent to random guessing.
Any classifier that succeeds on centered data must be using correlational structure, not templates.

\textbf{Coupling learning on centered data.}
Using the gradient formula (\ref{eq:J_grad}) with thermal pixel exchanges, we learn symmetric couplings $J_{ij}$ on the centered data.
The coupling gradient captures how exchange residuals at pixel $i$ correlate with values at pixel $j$:
\begin{equation}
\Delta J_{ij} \propto -\frac{1}{2k_{\rm B}T}\left( \text{residual}_i \cdot x_j + \text{residual}_j \cdot x_i \right).
\end{equation}
...
Nearest Centroid (centered data) & 9.4\% (random) \\
Coupling learning $J_{ij}$ (centered data) & \textbf{78.8\%} \\
\bottomrule
\end{tabular}
\end{center}
The key result: on data where first-order methods completely fail (9.4\%), coupling learning achieves 78.8\% accuracy.
This proves the method extracts genuine second-order correlational structure from the exchange dynamics.

Figure~\ref{fig:thermalization} shows the thermal pixel exchange process: a digit thermalizes while conserving total mass, with pixels gradually mixing while maintaining the global intensity.
Figure~\ref{fig:couplings} shows the learned coupling matrices $J_{ij}$ for each class, visualized as the coupling strength from the center pixel to all other pixels.
Unlike bias templates, these show local correlation patterns rather than digit shapes---confirming that couplings encode second-order structure.

\begin{figure}[h]
\centering
\includegraphics[width=\textwidth]{paper_figures/thermalization_demo.png}
\caption{Thermal pixel exchange dynamics. A digit ``3'' thermalizes over 100 timesteps while conserving total mass (7.13 $\to$ 7.13). Structure gradually dissolves as pixels exchange intensity, but the exchange statistics encode class-specific correlations that the coupling learning extracts.}
\label{fig:thermalization}
\end{figure}

\begin{figure}[h]
\centering
\includegraphics[width=\textwidth]{paper_figures/exchange_learned_J_spatial.png}
\caption{Learned couplings $J_{ij}$ for each digit class, showing the coupling strength from the center pixel to all other pixels. Blue indicates positive coupling (correlated pixels), red indicates negative (anti-correlated). Unlike bias templates which resemble digit shapes, the coupling patterns show local correlation structure---confirming that the 78.8\% accuracy on centered data comes from genuine second-order learning, not disguised template matching.}
\label{fig:couplings}
\end{figure}

\begin{figure}[h]
\centering
\includegraphics[width=\textwidth]{paper_figures/exchange_learning_curves.png}
\caption{Learning curves for coupling-based classification. Left: Train and test accuracy over 100 epochs, showing rapid convergence to $\sim$78.8\% test accuracy. The gap between train ($\sim$86\%) and test accuracy suggests room for regularization. Right: Total coupling norm $\|J\|$ grows linearly with training, indicating the optimizer continues to strengthen discriminative couplings.}
\label{fig:learning_curves}
\end{figure}

\subsection{Contrastive Learning: Further Separating Constraint Surfaces}

Coupling learning achieves 78.8\% test accuracy on centered data by learning correlational structure from exchange dynamics.
...
With coupling learning achieving 78.8\% test accuracy on centered data (where first-order methods fail entirely), the system demonstrates genuine second-order learning.
Contrastive fine-tuning may further improve accuracy, but the coupling result proves that the physics-based approach extracts correlational structure beyond simple templates.
...
\bibitem{whitelam2025training}
S.~Whitelam,
``Training thermodynamic computers by gradient descent,''
arXiv:2509.15324, 2025.

\bibitem{whitelam2025generative}
S.~Whitelam,
``Generative thermodynamic computing,''
arXiv:2506.15121, 2025.
