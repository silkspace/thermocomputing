...
\textbf{Attractor visualization:} Starting from uniform gray ($x_i = 0$) and evolving under $V_k$, pixels organize into the pattern for digit $k$.
...
The learned class biases $-\mathbf{b}_k$ function as attractor templates analogous to Hopfield stored patterns, resembling smoothed digit templates where positive (negative) bias values attract pixels toward white (black).
The contrastive signal $-b_k + \frac{1}{K-1}\sum_{j\neq k} b_j$ reveals what makes each digit \emph{unique}---the discriminative features that separate it from other classes.
